    ...
    @IBAction func action(_ sender: UIButton) {
        ...
        Task.init {
            ...
            if (actionButton.title(for: .normal) == "Encrypt") {
                ...
                result = await sdk.decrypt(ciphertext: self.textView.text, protocolID: PROTOCOL_ID, keyID: KEY_ID, counterparty: getCounterparty())
                if (result == "") {
                    // Create a new alert
        ...
        if metadataObj.type == AVMetadataObject.ObjectType.qr {
            if let outputString = metadataObj.stringValue {
                ...
                if (isSelectingRecipient) {
                    ...
                    Task.init {
                        // TODO: Add try catch and show error!
                        textView.text = await sdk.decrypt(ciphertext: outputString, protocolID: PROTOCOL_ID, keyID: KEY_ID, counterparty: getCounterparty())
                    }
                }
            ...
            if message=="" {
                ...
                Task.init {
                    if (isSelectingRecipient) {
                        ...
                        let decryptedMessage = await sdk.decrypt(ciphertext: message, protocolID: PROTOCOL_ID, keyID: KEY_ID, counterparty: getCounterparty())
